---
title: "Mon premier article toto"
subtitle: "Ceci est un sous-titre"
tags: ["javascript","webdev"]
category: "Tech"
published: true
---










csharp (highlight-start)
int x = 0;
// Cette ligne sera mise en évidence
int y = 1;
(highlight-end)


# Exemple d'article

Voici un exemple de code C# :

```csharp
.prose pre {
  padding: 1rem;
  border-radius: 0.5rem;
  background-color: #0f0f0f; /* Ajuster selon votre thème */
  color: #d1d5db; /* Couleur du texte */
  overflow-x: auto;
}

.prose code {
  background-color: #27272a;
  color: #facc15;
  padding: 0.2rem 0.4rem;
  border-radius: 0.25rem;
}

.highlighted {
  background-color: rgba(255, 255, 255, 0.1); /* Lignes surlignées */
}

.word-highlighted {
  background-color: rgba(255, 255, 0, 0.2); /* Mots surlignés */
}

```
fff


![f](data:image/png;base64,)





